...
class Services extends AppModel
{
     ...
     */
    public function getAllRenewablePaid($date = null, $include_failed_renewals = false, $future_attempts = false)
    {
        return $this->getRenewablePaidQuery($include_failed_renewals, $future_attempts)->fetchAll();
    }
    ...
    public function getRenewablePaidList(
        ...
        return $this->getRenewablePaidQuery($include_failed_renewals, $future_attempts)->
            order($order_by)->
            limit($this->getPerPage(), (max(1, $page) - 1) * $this->getPerPage())->
            fetchAll();
    }
     ...
     * @return array A list of services that have been paid and may be processed
     */
    public function getRenewablePaidCount($include_failed_renewals = false, $future_attempts = false)
    {
        return $this->getRenewablePaidQuery($include_failed_renewals, $future_attempts)->numResults();
    }
     ...
     */
    public function getRenewablePaidQuery($include_failed_renewals = false, $future_attempts = false)
    {
        ...
        if ($module_data) {
            ...
            if ($module) {
                ...
                if (($errors = $module->errors())) {
                    ...
                    $first_renewal_attempt_threshold = $this->Clients->getSetting(
                        $service->client_id,
                        'first_renewal_attempt_threshold'
                    );
                    $first_renewal_attempt_threshold = $first_renewal_attempt_threshold->value ?? 0;

                    if (
                        $service_invoice->failed_attempts >= $first_renewal_attempt_threshold
                            && !empty($first_renewal_attempt_threshold)
                    ) {
                        $first_renewal_attempt_spacing = $this->Clients->getSetting(
                            $service->client_id,
                            'first_renewal_attempt_spacing'
                        );
                        $params['date_next_attempt'] = $this->Date->modify(
                            date('c'),
                            '+' . (int) abs($first_renewal_attempt_spacing->value ?? 0) . ' hours',
                            'Y-m-d H:i:s',
                            Configure::get('Blesta.company_timezone')
                        );
                    }
                    ...
                    $second_renewal_attempt_threshold = $this->Clients->getSetting(
                        $service->client_id,
                        'second_renewal_attempt_threshold'
                    );
                    $second_renewal_attempt_threshold = $second_renewal_attempt_threshold->value ?? 0;

                    if (
                        $service_invoice->failed_attempts >= $second_renewal_attempt_threshold
                        && !empty($second_renewal_attempt_threshold)
                    ) {
                        $second_renewal_attempt_spacing = $this->Clients->getSetting(
                            $service->client_id,
                            'second_renewal_attempt_spacing'
                        );
                        $params['date_next_attempt'] = $this->Date->modify(
                            date('c'),
                            '+' . (int) abs($second_renewal_attempt_spacing->value ?? 0) . ' hours',
                            'Y-m-d H:i:s',
                            Configure::get('Blesta.company_timezone')
                        );
                    }
